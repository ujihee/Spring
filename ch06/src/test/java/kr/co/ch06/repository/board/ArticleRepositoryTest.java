...
class ArticleRepositoryTest {
    ...
    @Test
    @Transactional // transaction 처리, 엔티티 연관관계에서 한번 이상의 sql 처리가 이루어 지기 때문에 단일 작업 처리를 위해 @transacion처리 , 안하면 no session 에러
    void selectArticle(){
        Optional<Article> optArticle = articleRepository.findById(1); //select * from article where
        ...
        if(optArticle.isPresent()){
            ...
            for(Comment comment : commentList){ //select * from comment where
                System.out.println(comment);
            }
